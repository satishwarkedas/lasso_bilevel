function [Lambda, Beta, Time, Beta0] = lassotest(n, p, s, beta_type, rho, mew, max_iter)
    global data
...
%     [B, fitinfo] = lasso(X,Y,'CV',2);
    ...
    [result_new, var_names_new, Time(:,2)] = solveLassoBilevelMIQP(data, start_vec);
    ...
    ul(1,:) = 0.01;
    ul(2,:) = 10;
    [ll(1,:), phi(1,:)] = solveLasso1(ul(1,:), data);
    [ll(2,:), phi(2,:)] = solveLasso1(ul(2,:), data);
    
    for iter=2:max_iter
%         disp(iter);
        [ll(iter,:), phi(iter,:)] = solveLasso1(ul(iter,:), data);
    %     [ul(iter+1,:), ll(iter+1,:), mew_mat(iter,1:iter), obj_val(iter+1,:)] = solveLassoBilevelMIQP_Dempe(ul, ll', mew_mat, phi, data, iter);
        [ul(iter+1,:), ll(iter+1,:), mew_mat(iter,1:iter), obj_val(iter+1,:), time(iter-1,:)] = solveLassoBilevelDempe(ul, ll, mew_mat, phi, data, iter);
    end
