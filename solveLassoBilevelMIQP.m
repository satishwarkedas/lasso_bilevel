function [res, nam, tEnd] = solveLassoBilevelMIQP(data, start_vec)
...
function [A, b, sense_vec] = genlinconst(data_trainX, data_trainY, param)
    ...
    % for beta_0 -> 
    % 2*(X_train(1)*X_train) x beta + 0 x eps + 1 x tau1(k) + (-1) x tau2k
    % + 0 x u + 0 x lambda = 2*X_train(1)*Y_train
    ...
    % 2*(X_train(k)'*X_train) x beta + 0 x eps + 1 x tau1(k) + (-1) x
    % tau2(k)
