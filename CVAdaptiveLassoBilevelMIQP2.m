<<<<<<< HEAD
function [res, nam, tEnd, Q, b, A, c] = CVAdaptiveLassoBilevelMIQP2(data, start_vec, folds)
...
%         A_lambda = 1;
    ...
    % M_1 made dependent on the OLS estimates
    M_1 = max(1./eps_weights(2:end,:))*max(eps_weights(2:end,:));
    for k=1:param-1
        A_beta = zeros(param,1)';
        A_eps = zeros(param-1,1)';
        A_tau1 = I_pos(:,k)';
    ...
    % M_2 made dependent on the OLS estimates
    M_2 = max(1./eps_weights(2:end,:))*max(eps_weights(2:end,:));
    for k=1:param-1
        A_beta = [0,I_neg(:,k)'];
        A_eps = I_pos(:,k)';
        A_tau1 = zeros(param-1,1)';
...
>>>>>>> c835512b3d131f76d7a91aec03d051c772c7ef55
